...
char *mx_rename_group(void *jobj, t_comm *connect) {
    ...
    mx_notify_group_renamed(connect, grp_id, (char *)name);
...
char *mx_add_to_group(void *jobj, t_comm *connect) {
    ...
    if ((code = mx_validate_invite(connect->db, j_add, gid)) == -2)
        return mx_json_string_code_type(500, INV);
    else if (code == -1)
        return mx_json_string_code_type(666, INV);

    res = add_to_group(connect->db, j_add, gid, 0);

    if (res)
        mx_notify_add_to_group(connect, j_add, gid);
