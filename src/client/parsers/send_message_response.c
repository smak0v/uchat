...
static void parse_message_response(json_object *jobj, t_glade *g) {
    ...
    if (!check_did_in_dialogues(g->dialogues, json_object_get_int(j_id))
        && !g->group && json_object_get_int(j_uid2) != -1) {
        add_dialog_to_dialogues(g, j_id, j_i ? j_i : j_uid2);
        if (json_object_object_get(jobj, "dnme"))
            add_dialog_to_gui(g, j_uid2, j_id,
                json_object_object_get(jobj, "dnme"));
        else
            add_dialog_to_gui(g, j_i ? j_i : j_uid2, j_id, j_name);
    }
}
